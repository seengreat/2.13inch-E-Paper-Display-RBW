...
class EPD_2Inch13():
    ...
    def whitescreen_all(self,BWdatas,RWdatas):
        ...
        for j in range(EPD_HEIGHT):
            for i in range(x_bytes):
                index = i+j*x_bytes
                if i == 0:
                    self.write_data(BWdatas[index]>>6|0XFC)
                else:
                    self.write_data((BWdatas[index-1]<<2)|(BWdatas[index]>>6))
        self.write_cmd(0x26) #write RAM for red(0)/white (1)
        for j in range(EPD_HEIGHT):
            for i in range(x_bytes):
                index = i+j*x_bytes
                if i == 0:
                    self.write_data(~(RWdatas[index]>>6|0XFC))
                else:
                    self.write_data(~((RWdatas[index-1]<<2)|(RWdatas[index]>>6)))
        self.update()

    def whitescreen_all_fast(self,BWdatas,RWdatas):
        x_bytes = EPD_WIDTH//8+1
        self.write_cmd(0x24)
        for j in range(EPD_HEIGHT):
            for i in range(x_bytes):
    ...
    def setramvalue_basemap(self,BWdatas,RWdatas):
        x_bytes = EPD_WIDTH//8+1
        self.write_cmd(0x24)
        for j in range(EPD_HEIGHT):
            for i in range(x_bytes):
